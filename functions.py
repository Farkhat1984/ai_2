...
def find_apartments_by_district(district):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT rooms, square, price, address, district, url FROM apartments WHERE LOWER(district) = LOWER(?) LIMIT 10",
        (district,))
    apartments = cursor.fetchall()
    conn.close()
    if apartments:
...
def contact_to_owner(agreement,text):
    """
    Return a command to contact the owner if the user agrees.
    """
    if agreement:
        return text
    else:
        return "User did not agree to contact the owner."